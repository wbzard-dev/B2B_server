...
exports.getInventory = async (req, res) => {
    try {
        const inventory = await DistributorInventory.find({ distributorId: req.user.entityId })
...
exports.getSalesHistory = async (req, res) => {
    try {
        const sales = await DistributorSale.find({ distributorId: req.user.entityId })
            .sort({ date: -1 });
        res.json(sales);
    } catch (err) {
        console.error(err.message);
...
exports.getDistributorSalesForCompany = async (req, res) => {
    try {
        if (req.user.entityType !== 'Company') {
            return res.status(403).json({ msg: 'Only companies can access this' });
        }
        ...
        if (distributor.companyId.toString() !== req.user.entityId.toString()) {
            return res.status(403).json({ msg: 'Distributor does not belong to your company' });
        }

        const sales = await DistributorSale.find({ distributorId: req.params.distributorId })
            .sort({ date: -1 });
        res.json(sales);
    } catch (err) {
        console.error(err.message);
