...
exports.createOrder = async (req, res) => {
    ...
    if (req.user.entityType !== 'Distributor') {
        return res.status(403).json({ msg: 'Only distributors can place orders' });
    }
    ...
    try {
        ...
        if (distributor.status !== 'Approved' && distributor.status !== 'Active') {
            return res.status(403).json({ msg: `Your account is currently ${distributor.status}. Please contact support.` });
        }
        ...
        for (const item of items) {
            ...
            if (product.companyId.toString() !== distributor.companyId.toString()) {
                return res.status(400).json({ msg: `Product ${product.name} does not belong to your supplier` });
            }
...
exports.payOrder = async (req, res) => {
    try {
        ...
        if (order.distributorId != req.user.entityId) {
            console.log('Auth Mismatch:', order.distributorId, '!==', req.user.entityId);
            return res.status(403).json({ msg: 'Not authorized - ID Mismatch' });
        }
